  ...
  render() {
    ...
    if (this.props.pages) {
      tabItems = this.props.pages.map((item, index) => (
        <Tab key={index} eventKey={parseInt(parseInt(index, 10) + 1, 10)} title={item.name}>
          <div>
            <Runtimer
              width="100%"
              height="100%"
    ...
    return (
      <div>
        {this.props.pages && <Tabs activeKey={this.state.key} onSelect={this.handleSelect} id="controlled-tab-example">
                               {tabItems}
                             </Tabs>}
