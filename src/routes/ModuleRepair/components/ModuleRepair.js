...
export default class ModuleRepair extends Component {

  static propTypes = {
    modulerepair: PropTypes.object.isRequired,
  ...
  componentWillMount() {
    if (this.props.location.query.user_id && this.props.location.query.module) {
      this.props.searchInfo(this.props.location.query.user_id, this.props.location.query.module);
    }
  }

  componentWillReceiveProps(nextProps) {
    const { submitFinish } = this.props;
    if (nextProps.modulerepair.refresh === true) {
      submitFinish();
  ...
  submit = () => {
    const redata = {
      user_id: 1,
      device_id: false,
      module_id: this.props.location.query.module,
  ...
  render() {
    ...
    if (this.props.modulerepair.catalog) {
      interIntem = this.props.modulerepair.catalog.map((item, index) => {
        if (item.is_readonly) {
    ...
    return (
      <div>
        <Panel header={"模块报修"} bsStyle="primary" />
        <Form horizontal>
          {this.props.modulerepair.catalog && interIntem}
        </Form>
        <Pager>
