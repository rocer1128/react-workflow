...
export default class Message extends Component {
  static propTypes = {
    ...
    submitWrong: PropTypes.func.isRequired,
  };
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.message.type === "type_1") {
      submitFinish();
      browserHistory.push(`/confirmdeviceinfo?user_id=${this.props.location.query.user_id}&repair_apply_id=${this.props.location.query.repair_apply_id}`);
    } else if (nextProps.message.type === "type_2") {
      submitFinish();
      browserHistory.push(`/stoppage?user_id=${this.props.location.query.user_id}&repair_apply_id=${this.props.location.query.repair_apply_id}&apply_type=${nextProps.message.type}`);
    } else if (nextProps.message.type === "WRONG") {
      submitFinish();
      browserHistory.push(`/deviceinfo?user_id=${nextProps.message.wrong.user_id}&code=${nextProps.message.wrong.device_code}`);
    }
  }

  goBack = () => {
    this.props.submitWrong(this.props.location.query.user_id, this.props.location.query.repair_apply_id);
  };

  submit = () => {
    this.props.submitInfo(this.props.location.query.user_id, this.props.location.query.repair_apply_id);
  };
  render() {
    let interIntem = {};
    this.interRef = [];

    if (this.props.message.catalog) {
      interIntem = this.props.message.catalog.map((item, index) => {
        if (item.is_readonly) {
    ...
    return (
      <div>
        ...
        <Form horizontal>{this.props.message.catalog && interIntem}</Form>
        <Pager>
          <Pager.Item previous onClick={this.submit}>
            确认报修
          </Pager.Item>
          <Pager.Item next onClick={this.goBack}>
            报修有误
          </Pager.Item>
        </Pager>
      </div>
