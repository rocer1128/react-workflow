...
export default class ModuleList extends Component {
  ...
  submit = () => {
    browserHistory.push(`/modulerepair?user_id=${this.props.location.query.user_id}&code=${this.props.location.query.code}&module=${this.interRef[0].value}`);
  }
  render() {
    let interIntem = {};
    this.interRef = [];

    if (this.props.modulelist.catalog) {
      interIntem = this.props.modulelist.catalog.map((item, index) => {
        if (item.is_readonly) {
    ...
    return (
      <div>
        <Panel header={"模块列表"} bsStyle="primary" />
        <Form horizontal>
          {this.props.modulelist.catalog && interIntem}
        </Form>
        <Pager>
